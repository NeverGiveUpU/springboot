...
/**
 * 动态数据源注册
 * 实现 ImportBeanDefinitionRegistrar 实现数据源注册,@Import导入
...
public class DataSourceRegister implements ImportBeanDefinitionRegistrar, EnvironmentAware {
    ...
    public void registerBeanDefinitions(AnnotationMetadata annotationMetadata, BeanDefinitionRegistry beanDefinitionRegistry) {
        ...
        // 绑定默认数据源参数 也就是主数据源。（获得了DataSource）
        ...
        // 设置bean的类型，此处RoutingDataSource是继承AbstractRoutingDataSource的实现类
    ...
    private Class<? extends DataSource> getDataSourceType(String typeStr) {
        ...
        try {
            if (StringUtils.hasLength(typeStr)) {
                ...
                // 默认为hikariCP数据连接池，与springboot默认数据源保持一致
